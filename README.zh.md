...
<h1>Halfmoon: 基于Go语言的超高效 AI 助手</h1>
...
> **Halfmoon** 是由 [矽速科技 (Sipeed)](https://sipeed.com) 发起的独立开源项目，完全使用 **Go 语言**从零编写——不是 OpenClaw、NanoBot 或其他项目的分支。

🦐 **Halfmoon** 是一个受 [NanoBot](https://github.com/HKUDS/nanobot) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个"自举"过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。
...
> **🚨 安全声明**
>
> - **无加密货币 (NO CRYPTO):** Halfmoon **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> - **官方域名:** 唯一的官方网站是 **[halfmoon.io](https://halfmoon.io)**，公司官网是 **[sipeed.com](https://sipeed.com)**。
> - **警惕:** 许多 `.ai/.org/.com/.net/...` 后缀的域名被第三方抢注，请勿轻信。
> - **注意:** Halfmoon 正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在 1.0 正式版发布前，请不要将其部署到生产环境中。
> - **注意:** Halfmoon 最近合并了大量 PR，近期版本可能内存占用较大 (10~20MB)，我们将在功能较为收敛后进行资源占用优化。

## 📢 新闻

2026-03-17 🚀 **v0.2.3 发布！** 系统托盘 UI（Windows & Linux）、子 Agent 状态查询 (`spawn_status`)、实验性 Gateway 热重载、Cron 安全门控，以及 2 项安全修复。Halfmoon 已达 **25K ⭐**！
...
2026-02-26 🎉 Halfmoon 仅 17 天突破 **20K Stars**！频道自动编排和能力接口上线。

<details>
<summary>更早的新闻...</summary>

2026-02-16 🎉 Halfmoon 一周内突破 12K Stars！社区维护者角色和 [路线图](ROADMAP.md) 正式发布。

2026-02-13 🎉 Halfmoon 4 天内突破 5000 Stars！项目路线图和开发者群组筹建中。

2026-02-09 🎉 **Halfmoon 正式发布！** 仅用 1 天构建，将 AI Agent 带入 $10 硬件与 <10MB 内存的世界。🦐 皮皮虾，我们走！
...
访问 **[halfmoon.io](https://halfmoon.io)** — 官网自动检测你的平台，提供一键下载，无需手动选择架构。
...
从 [halfmoon.io](https://halfmoon.io) 下载后，双击 `halfmoon-launcher`（Windows 上为 `halfmoon-launcher.exe`），浏览器将自动打开 `http://localhost:18800`。
...
> **Docker / 虚拟机用户：** Gateway 默认监听 `127.0.0.1`。设置 `HALFMOON_GATEWAY_HOST=0.0.0.0` 或使用 `-public` 参数以允许从宿主机访问。
...
pkg install proot
termux-chroot ./halfmoon onboard   # chroot 提供标准 Linux 文件系统布局
```
...
对于只有 `halfmoon` 核心二进制文件的极简环境（无 Launcher UI），可通过命令行和 JSON 配置文件完成所有配置。
...
此命令会创建 `~/.halfmoon/config.json` 和工作区目录。

**2. 配置** (`~/.halfmoon/config.json`)
...
Halfmoon 通过 `model_list` 配置支持 30+ LLM Provider，使用 `协议/模型` 格式：
...
Halfmoon 可以搜索网络以提供最新信息。在 `tools.web` 中配置：
...
Halfmoon 内置文件操作、代码执行、定时任务等工具。详情请参阅 [工具配置](docs/zh/tools_configuration.md)。
...
Halfmoon 原生支持 [MCP](https://modelcontextprotocol.io/) — 连接任意 MCP 服务器，通过外部工具和数据源扩展 Agent 能力。
...
通过 CLI 或任何已集成的聊天应用发送一条消息，即可将 Halfmoon 连接到 Agent 社交网络。
...
| 命令                       | 说明                   |
| ------------------------- | ---------------------- |
| `halfmoon onboard`        | 初始化配置与工作区       |
| `halfmoon auth weixin` | 扫码连接微信个人号 |
| `halfmoon agent -m "..."` | 与 Agent 对话           |
| `halfmoon agent`          | 交互式对话模式           |
| `halfmoon gateway`        | 启动网关                |
| `halfmoon status`         | 查看状态                |
| `halfmoon version`        | 查看版本信息             |
| `halfmoon model`          | 查看或切换默认模型       |
| `halfmoon cron list`      | 列出所有定时任务         |
| `halfmoon cron add ...`   | 添加定时任务             |
| `halfmoon cron disable`   | 禁用定时任务             |
| `halfmoon cron remove`    | 删除定时任务             |
| `halfmoon skills list`    | 列出已安装 Skills        |
| `halfmoon skills install` | 安装 Skill              |
| `halfmoon migrate`        | 从旧版本迁移数据         |
| `halfmoon auth login`     | 认证 Provider           |

### ⏰ 定时任务 / 提醒

Halfmoon 通过 `cron` 工具支持定时提醒和重复任务：

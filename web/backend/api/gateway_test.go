...
import (
	...
	"github.com/halfmoon-labs/halfmoon/pkg/auth"
	"github.com/halfmoon-labs/halfmoon/pkg/config"
	"github.com/halfmoon-labs/halfmoon/web/backend/utils"
)
...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-halfmoon")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("HALFMOON_BINARY", invalidBinaryPath)
...
func TestFindHalfmoonBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "halfmoon-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("HALFMOON_BINARY", mockBinary)

	got := utils.FindHalfmoonBinary()
	if got != mockBinary {
		t.Errorf("FindHalfmoonBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindHalfmoonBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When HALFMOON_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("HALFMOON_BINARY", "/nonexistent/halfmoon-binary")

	got := utils.FindHalfmoonBinary()
	// Should not return the invalid path; falls back to "halfmoon" or another found path
	if got == "/nonexistent/halfmoon-binary" {
		t.Errorf("FindHalfmoonBinary() returned invalid env path %q, expected fallback", got)
	}
}

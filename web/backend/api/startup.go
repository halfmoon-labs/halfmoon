...
const (
	autoStartEntryName = "HalfmoonLauncher"
	launchAgentLabel   = "io.halfmoon.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "halfmoon-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=Halfmoon Web",
			"Comment=Start Halfmoon Web on login",

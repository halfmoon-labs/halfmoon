...
// HALFMOON_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "halfmoon_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("HALFMOON_SSH_KEY_PATH", keyPath)
}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "halfmoon-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".halfmoon", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithPicoclawHome(t *testing.T) {
	t.Setenv("HALFMOON_HOME", "/custom/halfmoon/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/halfmoon/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with HALFMOON_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
...
`
	...
	t.Setenv("HALFMOON_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("HALFMOON_SSH_KEY_PATH", "")
...
// unchanged when HALFMOON_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("HALFMOON_KEY_PASSPHRASE", "")
	t.Setenv("HALFMOON_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when HALFMOON_KEY_PASSPHRASE

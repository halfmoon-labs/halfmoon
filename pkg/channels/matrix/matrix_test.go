...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("halfmoon")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@halfmoon hello", want: true},
		{text: "hi @halfmoon:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bhalfmoon\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@halfmoon:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@halfmoon:matrix.org hello", want: "hello"},
		{in: "@halfmoon, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@halfmoon:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@halfmoon:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@halfmoon:matrix.org">Halfmoon</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40halfmoon%3Amatrix.org">Halfmoon</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@halfmoon:matrix.test"), "")
	if err != nil {

...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via HALFMOON_KEY_PASSPHRASE
...
//  2. HALFMOON_SSH_KEY_PATH env var
//  3. ~/.ssh/halfmoon_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "HALFMOON_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading HALFMOON_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "HALFMOON_SSH_KEY_PATH"

// halfmoonHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const halfmoonHome = "HALFMOON_HOME"

const (
	fileScheme = "file://"
	encScheme  = "enc://"
	hkdfInfo   = "halfmoon-credential-v1"
	saltLen    = 16
	nonceLen   = 12
	keyLen     = 32
)
...
//
// passphrase is required (HALFMOON_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// HALFMOON_SSH_KEY_PATH env var or ~/.ssh/halfmoon_ed25519.key.
...
//   - exact match with HALFMOON_SSH_KEY_PATH env var
//   - within the HALFMOON_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with HALFMOON_SSH_KEY_PATH.
	...
	// Within HALFMOON_HOME.
	if picoHome := os.Getenv(halfmoonHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="halfmoon-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set HALFMOON_SSH_KEY_PATH or place key at ~/.ssh/halfmoon_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (HALFMOON_SSH_KEY_PATH, HALFMOON_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. HALFMOON_SSH_KEY_PATH env var
//  3. ~/.ssh/halfmoon_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the halfmoon-specific SSH key path if it exists.

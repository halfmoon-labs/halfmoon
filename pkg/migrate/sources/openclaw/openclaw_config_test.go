...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.halfmoon/workspace" {
		t.Errorf("expected workspace '~/.halfmoon/workspace', got '%s'", workspace)
	}
...
func TestConvertToHalfmoon(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToHalfmoon("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.halfmoon/workspace" {
		t.Errorf("expected workspace '~/.halfmoon/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToHalfmoonWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToHalfmoon("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToHalfmoonWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToHalfmoonWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.halfmoon/workspace" {
		t.Errorf("expected workspace '~/.halfmoon/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}

...
<h1>Halfmoon: Assistente IA Ultra-Efficiente in Go</h1>

<h3>Hardware da $10 · 10MB di RAM · Avvio in ms · Let's Go, Halfmoon!</h3>
  <p>
...
> **Halfmoon** è un progetto open-source indipendente avviato da [Sipeed](https://sipeed.com), scritto interamente in **Go** da zero — non è un fork di OpenClaw, NanoBot o di qualsiasi altro progetto.

**Halfmoon** è un assistente IA personale ultra-leggero ispirato a [NanoBot](https://github.com/HKUDS/nanobot). È stato riscritto da zero in **Go** attraverso un processo di "auto-bootstrapping" — l'Agent IA stesso ha guidato la migrazione architetturale e l'ottimizzazione del codice.
...
> **Avviso di Sicurezza**
>
> * **NESSUNA CRYPTO:** Halfmoon **non** ha emesso token o criptovalute ufficiali. Qualsiasi annuncio su `pump.fun` o altre piattaforme di trading è una **truffa**.
> * **DOMINIO UFFICIALE:** L'**UNICO** sito ufficiale è **[halfmoon.io](https://halfmoon.io)**, e il sito aziendale è **[sipeed.com](https://sipeed.com)**
> * **ATTENZIONE:** Molti domini `.ai/.org/.com/.net/...` sono stati registrati da terze parti. Non fidarti di essi.
> * **NOTA:** Halfmoon è in fase di sviluppo iniziale rapido. Potrebbero esserci problemi di sicurezza non risolti. Non distribuire in produzione prima della v1.0.
> * **NOTA:** Halfmoon ha recentemente unito molte PR. Le build recenti potrebbero usare 10-20MB di RAM. L'ottimizzazione delle risorse è pianificata dopo la stabilizzazione delle funzionalità.

## 📢 Novità

2026-03-17 🚀 **v0.2.3 rilasciata!** Interfaccia system tray (Windows & Linux), query sullo stato dei sub-agent (`spawn_status`), hot-reload sperimentale del Gateway, gate di sicurezza per Cron e 2 correzioni di sicurezza. Halfmoon raggiunge **25K Stars**!
...
2026-02-26 🎉 Halfmoon raggiunge **20K stelle** in soli 17 giorni! Orchestrazione automatica dei canali e interfacce di capacità sono attive.

<details>
<summary>Notizie precedenti...</summary>

2026-02-16 🎉 Halfmoon supera 12K stelle in una settimana! Ruoli di maintainer della community e [Roadmap](ROADMAP.md) pubblicati ufficialmente.

2026-02-13 🎉 Halfmoon supera 5000 stelle in 4 giorni! Roadmap del progetto e gruppi sviluppatori in fase di avvio.

2026-02-09 🎉 **Halfmoon lanciato!** Costruito in 1 giorno per portare gli AI Agent su hardware da $10 con <10MB di RAM. Let's Go, Halfmoon!
...
Halfmoon può essere distribuito su quasi qualsiasi dispositivo Linux!
...
### Scarica da halfmoon.io (Consigliato)

Visita **[halfmoon.io](https://halfmoon.io)** — il sito ufficiale rileva automaticamente la tua piattaforma e fornisce il download con un clic. Non è necessario scegliere manualmente l'architettura.
...
Dopo aver scaricato da [halfmoon.io](https://halfmoon.io), fai doppio clic su `halfmoon-launcher` (o `halfmoon-launcher.exe` su Windows). Il browser si aprirà automaticamente su `http://localhost:18800`.
...
> **Utenti Docker / VM:** Il Gateway ascolta su `127.0.0.1` per impostazione predefinita. Imposta `HALFMOON_GATEWAY_HOST=0.0.0.0` o usa il flag `-public` per renderlo accessibile dall'host.
...
Dai una seconda vita al tuo telefono di dieci anni fa! Trasformalo in un assistente IA intelligente con Halfmoon.
...
pkg install proot
termux-chroot ./halfmoon onboard   # chroot fornisce un layout standard del filesystem Linux
```
...
Per ambienti minimali dove è disponibile solo il binario core `halfmoon` (senza Launcher UI), puoi configurare tutto tramite riga di comando e un file di configurazione JSON.
...
Questo crea `~/.halfmoon/config.json` e la directory workspace.

**2. Configura** (`~/.halfmoon/config.json`)
...
```bash
# Domanda singola
halfmoon agent -m "Quanto fa 2+2?"
...
Halfmoon supporta 30+ provider LLM tramite la configurazione `model_list`. Usa il formato `protocollo/modello`:
...
Parla con il tuo Halfmoon attraverso 17+ piattaforme di messaggistica:
...
Halfmoon può cercare sul web per fornire informazioni aggiornate. Configura in `tools.web`:
...
Halfmoon include strumenti integrati per operazioni su file, esecuzione di codice, pianificazione e altro. Vedi [Configurazione degli Strumenti](docs/tools_configuration.md) per i dettagli.
...
Halfmoon supporta nativamente [MCP](https://modelcontextprotocol.io/) — connetti qualsiasi server MCP per estendere le capacità del tuo Agent con strumenti e sorgenti di dati esterni.
...
Connetti Halfmoon al Social Network degli Agent semplicemente inviando un singolo messaggio tramite CLI o qualsiasi app di chat integrata.
...
| Comando                   | Descrizione                        |
| ------------------------- | ---------------------------------- |
| `halfmoon onboard`        | Inizializza config & workspace     |
| `halfmoon auth weixin` | Connetti account WeChat tramite QR |
| `halfmoon agent -m "..."` | Chatta con l'agent                 |
| `halfmoon agent`          | Modalità chat interattiva          |
| `halfmoon gateway`        | Avvia il gateway                   |
| `halfmoon status`         | Mostra lo stato                    |
| `halfmoon version`        | Mostra le info sulla versione      |
| `halfmoon model`          | Visualizza o cambia il modello predefinito |
| `halfmoon cron list`      | Elenca tutti i job pianificati     |
| `halfmoon cron add ...`   | Aggiunge un job pianificato        |
| `halfmoon cron disable`   | Disabilita un job pianificato      |
| `halfmoon cron remove`    | Rimuove un job pianificato         |
| `halfmoon skills list`    | Elenca le skill installate         |
| `halfmoon skills install` | Installa una skill                 |
| `halfmoon migrate`        | Migra i dati dalle versioni precedenti |
| `halfmoon auth login`     | Autenticazione con i provider      |

### ⏰ Task Pianificati / Promemoria

Halfmoon supporta promemoria pianificati e task ricorrenti tramite lo strumento `cron`:

...
<h1>Halfmoon: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>

<h3>Phần cứng $10 · RAM 10MB · Khởi động ms · Let's Go, Halfmoon!</h3>
  <p>
...
> **Halfmoon** là một dự án mã nguồn mở độc lập do [Sipeed](https://sipeed.com) khởi xướng, được viết hoàn toàn bằng **Go** từ đầu — không phải fork của OpenClaw, NanoBot hay bất kỳ dự án nào khác.

**Halfmoon** là trợ lý AI cá nhân siêu nhẹ lấy cảm hứng từ [NanoBot](https://github.com/HKUDS/nanobot). Nó được xây dựng lại từ đầu bằng **Go** thông qua quá trình "tự khởi động" — chính AI Agent đã dẫn dắt quá trình di chuyển kiến trúc và tối ưu hóa mã nguồn.
...
> **Thông báo Bảo mật**
>
> * **KHÔNG CÓ CRYPTO:** Halfmoon **chưa** phát hành bất kỳ token hay tiền điện tử chính thức nào. Mọi thông tin trên `pump.fun` hoặc các nền tảng giao dịch khác đều là **lừa đảo**.
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[halfmoon.io](https://halfmoon.io)**, và website công ty là **[sipeed.com](https://sipeed.com)**
> * **CẢNH BÁO:** Nhiều domain `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký. Đừng tin tưởng chúng.
> * **LƯU Ý:** Halfmoon đang trong giai đoạn phát triển nhanh. Có thể còn các vấn đề bảo mật chưa được giải quyết. Không triển khai lên môi trường production trước v1.0.
> * **LƯU Ý:** Halfmoon gần đây đã merge nhiều PR. Các bản build gần đây có thể dùng 10-20MB RAM. Tối ưu hóa tài nguyên được lên kế hoạch sau khi tính năng ổn định.

## 📢 Tin tức

2026-03-17 🚀 **v0.2.3 đã phát hành!** Giao diện system tray (Windows & Linux), truy vấn trạng thái sub-agent (`spawn_status`), thử nghiệm Gateway hot-reload, bảo mật Cron, và 2 bản vá bảo mật. Halfmoon đã đạt **25K Stars**!
...
2026-02-26 🎉 Halfmoon đạt **20K Stars** chỉ trong 17 ngày! Tự động điều phối Channel và giao diện khả năng đã hoạt động.

<details>
<summary>Tin tức trước đó...</summary>

2026-02-16 🎉 Halfmoon vượt 12K Stars trong một tuần! Vai trò người duy trì cộng đồng và [Lộ trình](ROADMAP.md) chính thức ra mắt.

2026-02-13 🎉 Halfmoon vượt 5000 Stars trong 4 ngày! Lộ trình dự án và nhóm nhà phát triển đang được xây dựng.

2026-02-09 🎉 **Halfmoon ra mắt!** Được xây dựng trong 1 ngày để đưa AI Agent lên phần cứng $10 với <10MB RAM. Let's Go, Halfmoon!
...
Halfmoon có thể được triển khai trên hầu hết mọi thiết bị Linux!
...
### Tải xuống từ halfmoon.io (Khuyến nghị)

Truy cập **[halfmoon.io](https://halfmoon.io)** — website chính thức tự động phát hiện nền tảng của bạn và cung cấp tải xuống một cú nhấp. Không cần chọn kiến trúc thủ công.
...
Sau khi tải xuống từ [halfmoon.io](https://halfmoon.io), nhấp đúp vào `halfmoon-launcher` (hoặc `halfmoon-launcher.exe` trên Windows). Trình duyệt của bạn sẽ tự động mở tại `http://localhost:18800`.
...
> **Người dùng Docker / VM:** Gateway lắng nghe trên `127.0.0.1` theo mặc định. Đặt `HALFMOON_GATEWAY_HOST=0.0.0.0` hoặc dùng cờ `-public` để có thể truy cập từ host.
...
Hãy cho chiếc điện thoại cũ của bạn một cuộc sống mới! Biến nó thành Trợ lý AI thông minh với Halfmoon.
...
Đối với các môi trường tối giản chỉ có binary lõi `halfmoon` (không có Launcher UI), bạn có thể cấu hình mọi thứ qua dòng lệnh và tệp cấu hình JSON.
...
Lệnh này tạo `~/.halfmoon/config.json` và thư mục workspace.

**2. Cấu hình** (`~/.halfmoon/config.json`)
...
Halfmoon hỗ trợ 30+ Provider LLM thông qua cấu hình `model_list`. Sử dụng định dạng `protocol/model`:
...
Trò chuyện với Halfmoon của bạn qua 17+ nền tảng nhắn tin:
...
Halfmoon có thể tìm kiếm web để cung cấp thông tin cập nhật. Cấu hình trong `tools.web`:
...
Halfmoon bao gồm các tool tích hợp sẵn cho thao tác tệp, thực thi mã, lên lịch và nhiều hơn nữa. Xem [Cấu hình Tools](docs/vi/tools_configuration.md) để biết chi tiết.
...
Halfmoon hỗ trợ [MCP](https://modelcontextprotocol.io/) gốc — kết nối bất kỳ MCP server nào để mở rộng khả năng Agent của bạn với các tool và nguồn dữ liệu bên ngoài.
...
Kết nối Halfmoon với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn duy nhất qua CLI hoặc bất kỳ Ứng dụng Chat nào đã tích hợp.
...
| Lệnh                      | Mô tả                                    |
| ------------------------- | ---------------------------------------- |
| `halfmoon onboard`        | Khởi tạo cấu hình & workspace           |
| `halfmoon auth weixin` | Kết nối tài khoản WeChat qua QR |
| `halfmoon agent -m "..."` | Trò chuyện với agent                     |
| `halfmoon agent`          | Chế độ trò chuyện tương tác             |
| `halfmoon gateway`        | Khởi động gateway                        |
| `halfmoon status`         | Hiển thị trạng thái                      |
| `halfmoon version`        | Hiển thị thông tin phiên bản            |
| `halfmoon model`          | Xem hoặc chuyển đổi mô hình mặc định   |
| `halfmoon cron list`      | Liệt kê tất cả công việc đã lên lịch   |
| `halfmoon cron add ...`   | Thêm công việc đã lên lịch             |
| `halfmoon cron disable`   | Vô hiệu hóa công việc đã lên lịch      |
| `halfmoon cron remove`    | Xóa công việc đã lên lịch              |
| `halfmoon skills list`    | Liệt kê các Skill đã cài đặt           |
| `halfmoon skills install` | Cài đặt một Skill                       |
| `halfmoon migrate`        | Di chuyển dữ liệu từ các phiên bản cũ  |
| `halfmoon auth login`     | Xác thực với các provider               |

### ⏰ Tác vụ Đã lên lịch / Nhắc nhở

Halfmoon hỗ trợ nhắc nhở đã lên lịch và tác vụ định kỳ thông qua tool `cron`:

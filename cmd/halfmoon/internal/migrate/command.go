...
import (
	...
	"github.com/halfmoon-labs/halfmoon/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to halfmoon",
		Args:  cobra.NoArgs,
		Example: `  halfmoon migrate
  halfmoon migrate --from openclaw
  halfmoon migrate --dry-run
  halfmoon migrate --refresh
  halfmoon migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.halfmoon)")

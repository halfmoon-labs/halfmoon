...
func getPidPath() string {
	...
	return filepath.Join(home, ".halfmoon", pidFileName)
}
...
func startGateway() error {
	...
	if runtime.GOOS == "windows" {
		cmd = exec.Command("cmd", "/C", "start /B halfmoon gateway > NUL 2>&1")
	} else {
		cmd = exec.Command("sh", "-c", "nohup halfmoon gateway > /dev/null 2>&1 & echo $! > "+pidPath)
	}
	...
	if runtime.GOOS == "windows" {
		cmd := exec.Command(
			...
			"process",
			"where",
			"name='halfmoon.exe' and commandline like '%gateway%'",
			"get",
			"processid",
		)
		output, err := cmd.Output()

...
Cấu hình công cụ của Halfmoon nằm trong trường `tools` của `config.json`.
...
| `base_url`     | string   | -        | URL cơ sở API tùy chỉnh, ví dụ: `https://litellm.example.com/v1` (tự động thêm `/chat/completions`) |
...
Theo mặc định, Halfmoon chặn các lệnh nguy hiểm sau:
...
Bộ bảo vệ exec chỉ xác thực lệnh cấp cao nhất được gửi đến Halfmoon. Nó **không** kiểm tra đệ quy các tiến trình con được tạo bởi các công cụ build hoặc script sau khi lệnh đó bắt đầu chạy.
...
Tất cả các tùy chọn cấu hình có thể được ghi đè qua biến môi trường với định dạng `HALFMOON_TOOLS_<SECTION>_<KEY>`:

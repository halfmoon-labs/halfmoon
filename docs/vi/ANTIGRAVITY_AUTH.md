...
**Antigravity** (Google Cloud Code Assist) là nhà cung cấp mô hình AI được Google hỗ trợ, cung cấp quyền truy cập vào các mô hình như Claude Opus 4.6 và Gemini thông qua hạ tầng đám mây của Google. Tài liệu này cung cấp hướng dẫn đầy đủ về cách xác thực hoạt động, cách lấy danh sách mô hình và cách triển khai nhà cung cấp mới trong Halfmoon.
...
9. [Cấu hình](#cấu-hình)
10. [Tạo nhà cung cấp mới trong Halfmoon](#tạo-nhà-cung-cấp-mới-trong-halfmoon)
...
- Go ≥ 1.25
- Mã nguồn Halfmoon (`pkg/providers/` và `pkg/auth/`)
...
Hồ sơ xác thực được lưu trữ trong `~/.halfmoon/auth.json`:
...
## Tạo nhà cung cấp mới trong Halfmoon

Các nhà cung cấp Halfmoon được triển khai dưới dạng gói Go trong `pkg/providers/`. Để thêm nhà cung cấp mới:
...
Nếu nhà cung cấp của bạn yêu cầu OAuth hoặc xác thực đặc biệt, thêm case vào `cmd/halfmoon/internal/auth/helpers.go`:
...
- **File nguồn:**
  ...
  - `pkg/auth/store.go` - Lưu trữ thông tin xác thực (`~/.halfmoon/auth.json`)
  ...
  - `cmd/halfmoon/internal/auth/helpers.go` - Lệnh CLI xác thực
...
- Làm mới token OAuth: `halfmoon auth login --provider antigravity`
...
- Kiểm tra lưu trữ hồ sơ xác thực: `~/.halfmoon/auth.json`
- Chạy lại `halfmoon auth login --provider antigravity`

...
設定ファイルパス: `~/.halfmoon/config.json`

### 環境変数

環境変数を使用してデフォルトパスを上書きできます。ポータブルインストール、コンテナ化デプロイ、または halfmoon をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HALFMOON_CONFIG` | 設定ファイルのパスを上書きします。halfmoon がどの `config.json` を読み込むかを直接指定し、他のすべての場所を無視します。 | `~/.halfmoon/config.json` |
| `HALFMOON_HOME`   | halfmoon データのルートディレクトリを上書きします。`workspace` やその他のデータディレクトリのデフォルト場所を変更します。          | `~/.halfmoon`             |

**例：**

```bash
# 特定の設定ファイルで halfmoon を実行
# ワークスペースパスはその設定ファイル内から読み込まれます
HALFMOON_CONFIG=/etc/halfmoon/production.json halfmoon gateway

# /opt/halfmoon にすべてのデータを保存して halfmoon を実行
# 設定はデフォルトの ~/.halfmoon/config.json から読み込まれます
# ワークスペースは /opt/halfmoon/workspace に作成されます
...
Halfmoon は設定されたワークスペース（デフォルト: `~/.halfmoon/workspace`）にデータを保存します：
...
1. `~/.halfmoon/workspace/skills`（ワークスペース）
2. `~/.halfmoon/skills`（グローバル）
...
Halfmoon はデフォルトでサンドボックス環境で実行されます。Agent は設定されたワークスペース内のファイルアクセスとコマンド実行のみが可能です。
...
| オプション              | デフォルト値            | 説明                                  |
| ----------------------- | ----------------------- | ------------------------------------- |
| `workspace`             | `~/.halfmoon/workspace` | Agent の作業ディレクトリ              |
...
exec セキュリティガードは Halfmoon が直接起動するコマンドラインのみを検査します。`make`、`go run`、`cargo`、`npm run`、またはカスタムビルドスクリプトなどの開発ツールが生成する子プロセスは再帰的に検査しません。
...
* ビルトインガードより強力な分離が必要な場合は、コンテナまたは VM 内で Halfmoon を実行してください。
...
Halfmoon は定期タスクを自動実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成してください：
...
| `model`    | `""`         | オプションのモデル指定（空 = デフォルトエージェントのモデルを使用） |

**環境変数:**

- `HALFMOON_HEARTBEAT_ENABLED=false` で無効化
- `HALFMOON_HEARTBEAT_INTERVAL=60` で間隔を変更
...
| `volcengine` | LLM（Volcengine 直接）                  | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Halfmoon&utm_content=Halfmoon&utm_medium=devrel&utm_source=OWO&utm_term=Halfmoon) |
...
> **新機能：** Halfmoon は**モデル中心**の設定アプローチを採用しました。`vendor/model` 形式（例：`zhipu/glm-4.7`）を指定するだけで新しい Provider を追加できます — **コード変更不要！**
...
| **VolcEngine (Doubao)** | `volcengine/`          | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI     | [取得](https://www.volcengine.com/activity/codingplan?utm_campaign=Halfmoon&utm_content=Halfmoon&utm_medium=devrel&utm_source=OWO&utm_term=Halfmoon) |
| **Antigravity**         | `antigravity/`         | Google Cloud                                        | Custom     | OAuth のみ                                                       |

#### ロードバランシング

同じモデル名に複数のエンドポイントを設定すると、Halfmoon が自動的にラウンドロビンします：
...
Halfmoon はプロトコルファミリーで Provider をルーティングします：
...
Halfmoon は `cron` ツールを通じて cron スタイルのスケジュールタスクをサポートします。
...
スケジュールタスクは再起動後も `~/.halfmoon/workspace/cron/` に保存されます。

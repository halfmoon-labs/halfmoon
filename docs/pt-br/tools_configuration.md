...
A configuração de ferramentas do Halfmoon está localizada no campo `tools` do `config.json`.
...
| `base_url`    | string   | -      | URL base da API personalizada, ex: `https://litellm.example.com/v1` (`/chat/completions` é adicionado automaticamente) |
...
Por padrão, o Halfmoon bloqueia os seguintes comandos perigosos:
...
O guarda exec apenas valida o comando de nível superior enviado ao Halfmoon. Ele **não** inspeciona recursivamente processos filhos gerados por ferramentas de build ou scripts após o início desse comando.
...
Todas as opções de configuração podem ser substituídas via variáveis de ambiente com o formato `HALFMOON_TOOLS_<SECTION>_<KEY>`:

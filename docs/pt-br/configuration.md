...
Arquivo de configuração: `~/.halfmoon/config.json`

### Variáveis de Ambiente

Você pode substituir os caminhos padrão usando variáveis de ambiente. Isso é útil para instalações portáteis, implantações em contêineres ou execução do halfmoon como serviço do sistema. Essas variáveis são independentes e controlam caminhos diferentes.

| Variável          | Descrição                                                                                                                             | Caminho Padrão              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HALFMOON_CONFIG` | Substitui o caminho para o arquivo de configuração. Isso indica diretamente ao halfmoon qual `config.json` carregar, ignorando todos os outros locais. | `~/.halfmoon/config.json` |
| `HALFMOON_HOME`   | Substitui o diretório raiz para dados do halfmoon. Isso altera o local padrão do `workspace` e outros diretórios de dados.          | `~/.halfmoon`             |

**Exemplos:**

```bash
# Executar halfmoon usando um arquivo de configuração específico
...
# Executar halfmoon com todos os dados armazenados em /opt/halfmoon
# A configuração será carregada do padrão ~/.halfmoon/config.json
# O workspace será criado em /opt/halfmoon/workspace
...
O Halfmoon armazena dados no seu workspace configurado (padrão: `~/.halfmoon/workspace`):
...
O Halfmoon é executado em um ambiente sandbox por padrão. O agente só pode acessar arquivos e executar comandos dentro do workspace configurado.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.halfmoon/workspace` | Diretório de trabalho do agente           |
...
O guard de segurança do exec inspeciona apenas a linha de comando que o Halfmoon executa diretamente. Ele não inspeciona recursivamente processos filhos gerados por ferramentas de desenvolvimento permitidas como `make`, `go run`, `cargo`, `npm run` ou scripts de build personalizados.
...
* Execute o Halfmoon dentro de um contêiner ou VM se precisar de isolamento mais forte do que o guard integrado oferece.
...
O Halfmoon pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `HALFMOON_HEARTBEAT_ENABLED=false` para desativar
* `HALFMOON_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
| `volcengine` | LLM (Volcengine direto)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Halfmoon&utm_content=Halfmoon&utm_medium=devrel&utm_source=OWO&utm_term=Halfmoon) |
...
> **Novidade:** Halfmoon agora usa uma abordagem **centrada no modelo**. Basta especificar o formato `vendor/model` (ex.: `zhipu/glm-4.7`) para adicionar novos providers — **sem alterações de código!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obter](https://www.volcengine.com/activity/codingplan?utm_campaign=Halfmoon&utm_content=Halfmoon&utm_medium=devrel&utm_source=OWO&utm_term=Halfmoon) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Somente OAuth                                                    |

#### Balanceamento de Carga

Configure múltiplos endpoints para o mesmo nome de modelo — Halfmoon fará round-robin automaticamente:
...
Halfmoon roteia providers por família de protocolo:
...
Halfmoon suporta tarefas agendadas via ferramenta `cron`.
...
As tarefas agendadas persistem após reinicializações em `~/.halfmoon/workspace/cron/`.

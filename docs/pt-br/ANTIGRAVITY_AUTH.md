...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no Halfmoon.
...
10. [Criando um Novo Provedor no Halfmoon](#criando-um-novo-provedor-no-halfmoon)
...
const antigravityPlugin = {
  ...
  register(api: HalfmoonPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<HalfmoonConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.25
- Base de código do Halfmoon (`pkg/providers/` e `pkg/auth/`)
...
Os perfis de autenticação são armazenados em `~/.halfmoon/auth.json`:
...
## Criando um Novo Provedor no Halfmoon

Os provedores do Halfmoon são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
...
Se seu provedor requer OAuth ou autenticação especial, adicione um caso em `cmd/halfmoon/internal/auth/helpers.go`:
...
halfmoon auth login --provider your-provider

# Listar modelos (para Antigravity)
halfmoon auth models

# Iniciar o gateway
halfmoon gateway

# Executar um agente com um modelo específico
halfmoon agent -m "Hello" --model your-model
```
...
export HALFMOON_AGENTS_DEFAULTS_MODEL=your-model

# Substituir configurações do provedor
export HALFMOON_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Arquivos Fonte:**
  ...
  - `pkg/auth/store.go` - Armazenamento de credenciais de autenticação (`~/.halfmoon/auth.json`)
  ...
  - `cmd/halfmoon/internal/auth/helpers.go` - Comandos CLI de autenticação
...
- Atualizar tokens OAuth: `halfmoon auth login --provider antigravity`
...
- Verificar o armazenamento do perfil de autenticação: `~/.halfmoon/auth.json`
- Executar novamente `halfmoon auth login --provider antigravity`

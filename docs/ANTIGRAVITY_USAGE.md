# Using Antigravity Provider in Halfmoon

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in Halfmoon.
...
6.  **Paste it back into the terminal** where Halfmoon is waiting.

Halfmoon will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.halfmoon/config.json` or override it via the CLI:
...
*   **429 Rate Limit**: Antigravity has strict quotas. Halfmoon will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `halfmoon auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.

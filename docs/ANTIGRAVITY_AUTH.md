...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Halfmoon.
...
10. [Creating a New Provider in Halfmoon](#creating-a-new-provider-in-halfmoon)
...
- Go ≥ 1.25
- Halfmoon codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.halfmoon/auth.json`:
...
## Creating a New Provider in Halfmoon

Halfmoon providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/halfmoon/internal/auth/helpers.go`:
...
- **Source Files:**
  ...
  - `pkg/auth/store.go` - Auth credential storage (`~/.halfmoon/auth.json`)
  - `pkg/providers/factory.go` - Provider factory and protocol routing
  - `pkg/providers/types.go` - Provider interface definitions
  - `cmd/halfmoon/internal/auth/helpers.go` - Auth CLI commands
...
### "Token expired"
- Refresh OAuth tokens: `halfmoon auth login --provider antigravity`
...
- Check auth profile storage: `~/.halfmoon/auth.json`
- Re-run `halfmoon auth login --provider antigravity`

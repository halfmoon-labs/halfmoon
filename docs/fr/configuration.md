...
Fichier de configuration : `~/.halfmoon/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de Halfmoon en tant que service système. Ces variables sont indépendantes et contrôlent des chemins différents.

| Variable          | Description                                                                                                                             | Chemin par défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HALFMOON_CONFIG` | Remplace le chemin vers le fichier de configuration. Indique directement à Halfmoon quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.halfmoon/config.json` |
| `HALFMOON_HOME`   | Remplace le répertoire racine des données Halfmoon. Change l'emplacement par défaut du `workspace` et des autres répertoires de données. | `~/.halfmoon`             |

**Exemples :**

```bash
# Run halfmoon using a specific config file
# The workspace path will be read from within that config file
HALFMOON_CONFIG=/etc/halfmoon/production.json halfmoon gateway

# Run halfmoon with all its data stored in /opt/halfmoon
# Config will be loaded from the default ~/.halfmoon/config.json
# Workspace will be created at /opt/halfmoon/workspace
HALFMOON_HOME=/opt/halfmoon halfmoon agent

# Use both for a fully customized setup
HALFMOON_HOME=/srv/halfmoon HALFMOON_CONFIG=/srv/halfmoon/main.json halfmoon gateway
```

### Structure du Workspace

Halfmoon stocke les données dans votre workspace configuré (par défaut : `~/.halfmoon/workspace`) :
...
1. `~/.halfmoon/workspace/skills` (workspace)
2. `~/.halfmoon/skills` (global)
...
```bash
export HALFMOON_BUILTIN_SKILLS=/path/to/skills
```
...
Halfmoon s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes que dans le workspace configuré.
...
| ----------------------- | ----------------------- | ------------------------------------------------- |
| `workspace`             | `~/.halfmoon/workspace` | Répertoire de travail de l'agent                  |
...
Le garde de sécurité exec n'inspecte que la ligne de commande lancée directement par Halfmoon. Il n'inspecte pas récursivement les processus enfants générés par les outils de développement autorisés tels que `make`, `go run`, `cargo`, `npm run` ou les scripts de build personnalisés.
...
* Exécutez Halfmoon dans un conteneur ou une VM si vous avez besoin d'une isolation plus forte que celle fournie par le garde intégré.
...
```bash
export HALFMOON_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
Halfmoon peut effectuer des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
| `volcengine` | LLM (Volcengine direct)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Halfmoon&utm_content=Halfmoon&utm_medium=devrel&utm_source=OWO&utm_term=Halfmoon) |
...
> **Nouveauté :** Halfmoon utilise désormais une approche **centrée sur le modèle**. Spécifiez simplement le format `vendor/model` (ex. `zhipu/glm-4.7`) pour ajouter de nouveaux providers — **aucune modification de code requise !**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obtenir](https://www.volcengine.com/activity/codingplan?utm_campaign=Halfmoon&utm_content=Halfmoon&utm_medium=devrel&utm_source=OWO&utm_term=Halfmoon) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | OAuth uniquement                                                 |

#### Équilibrage de Charge

Configurez plusieurs endpoints pour le même nom de modèle — Halfmoon effectuera automatiquement un round-robin :
...
Halfmoon route les providers par famille de protocole :
...
Halfmoon supporte les tâches planifiées via l'outil `cron`. L'agent peut définir, lister et annuler des rappels ou tâches récurrentes.
...
Les tâches planifiées persistent après redémarrage dans `~/.halfmoon/workspace/cron/`.

...
Halfmoon prend en charge le chiffrement des valeurs `api_key` dans les entrées de configuration `model_list`.
...
```bash
export HALFMOON_KEY_PASSPHRASE="your-passphrase"
```

**2. Chiffrer une clé API**

Exécutez `halfmoon onboard` — il vous demande votre phrase secrète et génère la clé SSH,
...
| Chiffré | `enc://<base64>` | Déchiffré au démarrage avec `HALFMOON_KEY_PASSPHRASE` |
...
aes_key = HKDF-SHA256(ikm, salt, info="halfmoon-credential-v1", 32 bytes)
```
...
1. La **phrase secrète** (`HALFMOON_KEY_PASSPHRASE`)
...
|----------|--------|-------------|
| `HALFMOON_KEY_PASSPHRASE` | Oui (pour `enc://`) | Phrase secrète utilisée pour la dérivation de clé |
| `HALFMOON_SSH_KEY_PATH` | Non | Chemin vers la clé privée SSH. Si non défini, détection automatique depuis `~/.ssh/halfmoon_ed25519.key` |

### Détection automatique de la clé SSH

Si `HALFMOON_SSH_KEY_PATH` n'est pas défini, Halfmoon recherche la clé dédiée :

```
~/.ssh/halfmoon_ed25519.key
```

Ce fichier dédié évite les conflits avec les clés SSH existantes de l'utilisateur.
Exécutez `halfmoon onboard` pour le générer automatiquement.
...
> **Remarque :** Un fichier de clé SSH est requis pour le chiffrement des identifiants. Si aucune clé n'est trouvée et que `HALFMOON_SSH_KEY_PATH` n'est pas défini, le chiffrement/déchiffrement échouera. Exécutez `halfmoon onboard` pour générer la clé automatiquement.

---

## Migration

Étant donné que les seuls éléments secrets sont `HALFMOON_KEY_PASSPHRASE` et le fichier de clé privée SSH, la migration est simple :

1. Copiez le fichier de configuration sur la nouvelle machine.
2. Définissez `HALFMOON_KEY_PASSPHRASE` avec la même valeur.
3. Copiez le fichier de clé privée SSH au même chemin (ou définissez `HALFMOON_SSH_KEY_PATH` vers son nouvel emplacement).
...
- **La phrase secrète et la clé SSH sont toutes deux requises.** La clé SSH agit comme un second facteur — sans elle, le chiffrement/déchiffrement échouera. Exécutez `halfmoon onboard` pour générer la clé si elle n'existe pas.
- **La clé SSH est en lecture seule à l'exécution.** Halfmoon n'écrit ni ne modifie jamais le fichier de clé SSH.
- **Les clés en texte clair restent prises en charge.** Les configurations existantes sans `enc://` ne sont pas affectées.
- **Le format `enc://` est versionné** via le champ `info` de HKDF (`halfmoon-credential-v1`), permettant de futures mises à niveau d'algorithme sans casser les valeurs chiffrées existantes.

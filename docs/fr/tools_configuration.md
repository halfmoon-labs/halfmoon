...
La configuration des outils de Halfmoon se trouve dans le champ `tools` de `config.json`.
...
| `base_url`    | string   | -          | URL de base API personnalisée, ex : `https://litellm.example.com/v1` (`/chat/completions` est ajouté automatiquement) |
...
Par défaut, Halfmoon bloque les commandes dangereuses suivantes :
...
Le garde exec ne valide que la commande de niveau supérieur envoyée à Halfmoon. Il n'inspecte **pas** récursivement les processus enfants générés par les outils de build ou les scripts après le démarrage de cette commande.
...
Toutes les options de configuration peuvent être remplacées via des variables d'environnement au format `HALFMOON_TOOLS_<SECTION>_<KEY>` :

...
# Utiliser le fournisseur Antigravity dans Halfmoon

Ce guide explique comment configurer et utiliser le fournisseur **Antigravity** (Google Cloud Code Assist) dans Halfmoon.
...
```bash
halfmoon auth login --provider antigravity
```

### Authentification manuelle (Headless/VPS)
Si vous exécutez Halfmoon sur un serveur (Coolify/Docker) et ne pouvez pas accéder à `localhost`, suivez ces étapes :
...
6.  **Collez-la dans le terminal** où Halfmoon attend.

Halfmoon extraira automatiquement le code d'autorisation et terminera le processus.
...
### Changer de modèle
Vous pouvez modifier le modèle par défaut dans `~/.halfmoon/config.json` ou le remplacer via le CLI :

```bash
# Remplacer pour une seule commande
halfmoon agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Variables d'environnement** :
    *   `HALFMOON_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Persistance de l'authentification** :
    ...
    ```bash
    scp ~/.halfmoon/auth.json user@your-server:~/.halfmoon/
    ```
...
*   **429 Limite de débit** : Antigravity a des quotas stricts. Halfmoon affichera le « temps de réinitialisation » dans le message d'erreur si vous atteignez une limite.
*   **404 Non trouvé** : Assurez-vous d'utiliser un ID de modèle provenant de la liste `halfmoon auth models`. Utilisez l'ID court (par ex. `gemini-3-flash`) et non le chemin complet.

...
10. [Créer un nouveau fournisseur dans Halfmoon](#créer-un-nouveau-fournisseur-dans-halfmoon)
...
- Go ≥ 1.25
- Base de code Halfmoon (`pkg/providers/` et `pkg/auth/`)
...
Les profils d'authentification sont stockés dans `~/.halfmoon/auth.json` :
...
## Créer un nouveau fournisseur dans Halfmoon

Les fournisseurs Halfmoon sont implémentés en tant que packages Go sous `pkg/providers/`. Pour ajouter un nouveau fournisseur :
...
Si votre fournisseur nécessite OAuth ou une authentification spéciale, ajoutez un cas dans `cmd/halfmoon/internal/auth/helpers.go` :
...
- **Fichiers source :**
  ...
  - `pkg/auth/store.go` - Stockage des identifiants d'authentification (`~/.halfmoon/auth.json`)
  ...
  - `cmd/halfmoon/internal/auth/helpers.go` - Commandes CLI d'authentification
...
- Rafraîchir les jetons OAuth : `halfmoon auth login --provider antigravity`
...
- Vérifier le stockage du profil d'authentification : `~/.halfmoon/auth.json`
- Relancer `halfmoon auth login --provider antigravity`

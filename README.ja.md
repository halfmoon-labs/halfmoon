<div align="center">
  ...
  <h1>Halfmoon: Go で書かれた超効率 AI アシスタント</h1>

  <h3>$10 ハードウェア · 10MB RAM · ms 起動 · Let's Go, Halfmoon!</h3>
  <p>
...
> **Halfmoon** は [Sipeed](https://sipeed.com) が立ち上げた独立したオープンソースプロジェクトです。完全に **Go 言語**で一から書かれており、OpenClaw、NanoBot、その他のプロジェクトのフォークではありません。

**Halfmoon** は [NanoBot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。**Go** でゼロからリビルドされ、「セルフブートストラッピング」プロセスで構築されました — AI Agent 自身がアーキテクチャの移行とコード最適化を推進しました。
...
> **セキュリティに関する注意**
>
> * **暗号通貨なし:** Halfmoon には公式トークン/コインは**一切ありません**。`pump.fun` やその他の取引プラットフォームでの主張はすべて**詐欺**です。
> * **公式ドメイン:** **唯一**の公式サイトは **[halfmoon.io](https://halfmoon.io)**、企業サイトは **[sipeed.com](https://sipeed.com)** です。
> * **注意:** 多くの `.ai/.org/.com/.net/...` ドメインは第三者によって登録されています。信頼しないでください。
> * **注記:** Halfmoon は初期開発段階にあり、未解決のネットワークセキュリティ問題がある可能性があります。v1.0 リリース前に本番環境へのデプロイは避けてください。
> * **注記:** Halfmoon は最近多くの PR をマージしており、最新バージョンではメモリフットプリントが大きくなる場合があります（10〜20MB）。機能セットが安定次第、リソース最適化を優先する予定です。

## 📢 ニュース

2026-03-17 🚀 **v0.2.3 リリース！** システムトレイ UI（Windows & Linux）、サブエージェントステータス追跡（`spawn_status`）、実験的 Gateway ホットリロード、cron セキュリティゲート、セキュリティ修正 2 件。Halfmoon **25K ⭐** 達成！
...
2026-02-26 🎉 Halfmoon がわずか 17 日で **20K スター** 達成！Channel 自動オーケストレーションとケイパビリティインターフェースが実装されました。

<details>
<summary>過去のニュース...</summary>

2026-02-16 🎉 Halfmoon が 1 週間で 12K スター達成！コミュニティメンテナーの役割と[ロードマップ](ROADMAP.md)が正式に公開されました。

2026-02-13 🎉 Halfmoon が 4 日間で 5000 スター達成！プロジェクトロードマップと開発者グループの準備が進行中。

2026-02-09 🎉 **Halfmoon リリース！** $10 ハードウェアで 10MB 未満の RAM で動く AI Agent を 1 日で構築。Let's Go, Halfmoon!
...
Halfmoon はほぼすべての Linux デバイスにデプロイできます！
...
### halfmoon.io からダウンロード（推奨）

**[halfmoon.io](https://halfmoon.io)** にアクセス — 公式サイトがプラットフォームを自動検出し、ワンクリックでダウンロードできます。アーキテクチャを手動で選ぶ必要はありません。
...
[halfmoon.io](https://halfmoon.io) からダウンロード後、`halfmoon-launcher`（Windows では `halfmoon-launcher.exe`）をダブルクリックしてください。ブラウザが自動的に `http://localhost:18800` を開きます。
...
> **Docker / VM ユーザー:** Gateway はデフォルトで `127.0.0.1` でリッスンします。ホストからアクセスできるようにするには `HALFMOON_GATEWAY_HOST=0.0.0.0` を設定するか、`-public` フラグを使用してください。
...
10 年前のスマホに第二の人生を！Halfmoon でスマート AI アシスタントに変身させましょう。
...
pkg install proot
termux-chroot ./halfmoon onboard   # chroot で標準的な Linux ファイルシステムレイアウトを提供
```
...
`halfmoon` コアバイナリのみが利用可能な最小環境（Launcher UI なし）では、コマンドラインと JSON 設定ファイルですべてを設定できます。
...
`~/.halfmoon/config.json` とワークスペースディレクトリが作成されます。

**2. 設定** (`~/.halfmoon/config.json`)
...
Halfmoon は `model_list` 設定を通じて 30 以上の LLM Provider をサポートしています。`protocol/model` 形式を使用してください：
...
17 以上のメッセージングプラットフォームで Halfmoon と会話できます：
...
Halfmoon は最新情報を提供するために Web を検索できます。`tools.web` で設定してください：
...
Halfmoon にはファイル操作、コード実行、スケジューリングなどの組み込みツールが含まれています。詳細は [ツール設定](docs/ja/tools_configuration.md) を参照してください。
...
Halfmoon は [MCP](https://modelcontextprotocol.io/) をネイティブサポートしています — 任意の MCP サーバーに接続して、外部ツールやデータソースで Agent の機能を拡張できます。
...
CLI または統合チャットアプリからメッセージを 1 つ送るだけで、Halfmoon をエージェントソーシャルネットワークに接続できます。
...
| コマンド                    | 説明                           |
| ------------------------- | ------------------------------ |
| `halfmoon onboard`        | 設定＆ワークスペースの初期化     |
| `halfmoon auth weixin` | WeChat アカウントを QR で接続 |
| `halfmoon agent -m "..."` | Agent とチャット                |
| `halfmoon agent`          | インタラクティブチャットモード   |
| `halfmoon gateway`        | Gateway を起動                  |
| `halfmoon status`         | ステータスを表示                |
| `halfmoon version`        | バージョン情報を表示            |
| `halfmoon model`          | デフォルトモデルの表示・切替    |
| `halfmoon cron list`      | スケジュールジョブ一覧          |
| `halfmoon cron add ...`   | スケジュールジョブを追加         |
| `halfmoon cron disable`   | スケジュールジョブを無効化       |
| `halfmoon cron remove`    | スケジュールジョブを削除         |
| `halfmoon skills list`    | インストール済み Skill 一覧      |
| `halfmoon skills install` | Skill をインストール             |
| `halfmoon migrate`        | 旧バージョンからデータを移行     |
| `halfmoon auth login`     | Provider への認証               |

### ⏰ スケジュールタスク / リマインダー

Halfmoon は `cron` ツールによるスケジュールリマインダーと定期タスクをサポートしています：

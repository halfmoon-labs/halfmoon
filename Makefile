...
CONFIG_PKG=github.com/halfmoon-labs/halfmoon/pkg/config
...
HALFMOON_HOME?=$(HOME)/.halfmoon
WORKSPACE_DIR?=$(HALFMOON_HOME)/workspace
...
build-launcher:
	@echo "Building halfmoon-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
...
## build-all: Build halfmoon for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install halfmoon to system and copy builtin skills
install: build
...
## uninstall: Remove halfmoon from system
uninstall:
...
## uninstall-all: Remove halfmoon and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(HALFMOON_HOME)
	@echo "Removed workspace: $(HALFMOON_HOME)"
...
vet: generate
	@packages="$$(go list ./...)" && \
		$(GO) vet $$(printf '%s\n' "$$packages" | grep -v '^github.com/halfmoon-labs/halfmoon/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $$(go list ./... | grep -v github.com/halfmoon-labs/halfmoon/web/)
	@cd web && make test
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build halfmoon-agent halfmoon-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build halfmoon-agent halfmoon-gateway
...
## docker-run: Run halfmoon gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run halfmoon gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run halfmoon agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm halfmoon-agent

## docker-run-agent-full: Run halfmoon agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm halfmoon-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi halfmoon:latest halfmoon:full 2>/dev/null || true


## build-macos-app: Build HalfMoon macOS .app bundle (no terminal window)
build-macos-app:
	...
	@echo "macOS .app bundle created: $(BUILD_DIR)/HalfMoon.app"

## help: Show this help message
help:
	@echo "halfmoon Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  make [target]"
	@echo ""
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.halfmoon/workspace)"
